...
function PN_API(setup) {
    ...
    function publish(next) {
        if (next)
            PUB_QUEUE.sending = 0;
        if (PUB_QUEUE.sending || !PUB_QUEUE.length)
            return;
        PUB_QUEUE.sending = 1;
        xdr(PUB_QUEUE.shift());
    }
    ...
    var SELF = {
        'LEAVE': function (channel, blocking) {
            ...
            if (channel.indexOf(PRESENCE_SUFFIX) > 0)
                return;
        ...
        'history': function (args, callback) {
            ...
            var busca = ORIGIN.search("https://");
            if (busca === -1) {
                var secureProtocol = ORIGIN.replace("http://", "https://");
            }
            ORIGIN = secureProtocol;
        ...
        'subscribe': function (args, callback) {
            ...
            function _connect() {
                ...
                SUB_RECEIVER = xdr({
                    ...
                    success: function (messages) {
                        if (!messages)
                            return timeout(_connect, windowing);
            ...
            if (!READY)
                return READY_BUFFER.push(CONNECT);
...
window['PUBNUB'] || (function () {
    ...
    function bind(type, el, fun) {
        each(type.split(','), function (etype) {
            ...
            if (el.addEventListener)
                el.addEventListener(etype, rapfun, false);
            else if (el.attachEvent)
                el.attachEvent('on' + etype, rapfun);
            else
                el[ 'on' + etype ] = rapfun;
        });
    }
    ...
    function unbind(type, el, fun) {
        if (el.removeEventListener)
            el.removeEventListener(type, false);
    ...
    function ajax(setup) {
        ...
        try {
            ...
            var reformUrl = url.replace("http", "https");
            //----
            xhr.open('GET', reformUrl, async);
            xhr.send();
        } catch (eee) {
            done(0);
            XORIGN = 0;
...
// PUBNUB Flash Socket
    ...
    if ('opera' in window || attr(PDIV, 'flash'))
        PDIV['innerHTML'] =
